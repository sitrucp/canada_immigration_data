...
def _validate_aggregates(long_df: pd.DataFrame, l01: pd.DataFrame, l12: pd.DataFrame, l23: pd.DataFrame) -> None:
    ...
    def map_l2(label: str) -> str:
        return label if label != "Vulnerable workers" else "Vulnerable workers (subcategory)"

    csv_agg_l2 = (long_df.dropna(subset=["category_2"])  # rows that have category_2
                          .assign(category_2=lambda d: d["category_2"].map(map_l2))
                          .groupby(["category_2"], as_index=False)["value"].sum())
    links_agg_l2 = (l12.assign(category_2=lambda d: d["category_2"].map(map_l2))
                       .groupby(["category_2"], as_index=False)["value"].sum())
...
def build_nodes_links(df: pd.DataFrame, drop_totals: bool = True):
    ...
    # Nodes - Add top-level IMP node
    level0 = ["IMP"]  # Top-level node
    ...
    # Fix the "Vulnerable workers" loop issue by renaming the category_2 version
    level2 = [label if label != "Vulnerable workers" else "Vulnerable workers (subcategory)" for label in level2]
    ...
    # Links
    # Level 0 to Level 1: IMP to all category_1 nodes (including "not stated" province)
    l01 = (long_df.dropna(subset=["category_1"])
                 .groupby(["province_territory","year","category_1"], as_index=False)["value"].sum())
    l01["source"] = l01["category_1"].map(lambda x: node_id_map["IMP"])  # All from IMP node
    l01["target"] = l01["category_1"].map(node_id_map)
    
    
    # Level 1 to Level 2: category_1 to category_2 (existing logic)
    ...
    # Fix the "Vulnerable workers" loop by renaming the target
    l12["target"] = l12["category_2"].map(lambda x: node_id_map.get(x if x != "Vulnerable workers" else "Vulnerable workers (subcategory)"))
    
    # Level 2 to Level 3: category_2 to category_3 (existing logic)
    ...
    # Fix the "Vulnerable workers" loop by renaming the source
    l23["source"] = l23["category_2"].map(lambda x: node_id_map.get(x if x != "Vulnerable workers" else "Vulnerable workers (subcategory)"))
...
def main():
    input_csv = "extracted_imp.csv"
    template_html = "sankey_imp_template.html"
    output_html = "sankey_imp.html"
