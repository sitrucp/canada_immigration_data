...
def build_echarts_data(df: pd.DataFrame, drop_totals: bool = True):
    ...
    # Use level2 as-is (collision handling now done at data extraction level)
...
def create_color_schema(df: pd.DataFrame) -> dict:
    """Create a color schema mapping for top-level nodes (direct children of IMP).
...
def main():
    input_csv = "goc_data_processed/extracted_imp.csv"
