...
def build_echarts_data(df: pd.DataFrame, drop_totals: bool = True):
    ...
    if "year" in df.columns and "value" in df.columns:
        ...
        # Unpivot
        id_vars = ["province_territory","claim_office_type"]
    ...
    l01["source"] = l01["claim_office_type"].map(lambda x: node_id_map["Asylum"])  # All from Asylum node
...
def create_color_schema(df: pd.DataFrame) -> dict:
    """Create a color schema mapping for top-level nodes (direct children of Asylum).
    ...
    category_values = df.groupby('claim_office_type')['value'].sum().sort_values(ascending=False)
...
def main():
    input_csv = "goc_data_processed/extracted_asylum.csv"
